...
bool Serial::OpenSerial(int nSpeed, char nEvent, int nBits, int nStop) {
    if (wait_uart) {
...
bool Serial::send_control(
    ...
    data.w = w;
    data.shoot = shoot;
    data.found_armor = found_armor;
    for(int i = 0; i<7;i++)
        data.reserved[i] = 0;
    data.end = 'E';
    return WriteData((const unsigned char *)&data, sizeof(toSTM32_t));
}
