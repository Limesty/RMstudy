...
struct pos{//机器人坐标
    ...
    float y;
    float z;
    quat rot;//表示机器人朝向的四元数，可用欧拉角转换
};
struct mapdata{
    pos origin;//地图原点坐标（即图片左下角像素相对坐标系的坐标）
    float width;//地图图片宽度
    float height;//地图图片高度
    float resolution;//地图图片分辨率
    std::vector<int> obstacle_data;//一维数组，储存障碍物信息
    /*
    数组索引与实际坐标的关系：index = (int)((x - origin_x) / resolution) + ((int)((y - origin_y) / resolution)) * width;
    0:空白区域
    100:障碍物
    -1:未知
...
class Blackboard {
 public:
  ...
  //地图模块接口
  mapdata get_map(int argc, char **argv); //argc,argv为命令行参数，用于初始化节点；此接口返回关于地图的信息
  pos get_curr_pos(int argc, char** argv);//argc,argv为命令行参数，用于初始化节点；此接口返回关于机器人当前坐标
  void send_one_goal(int argc,char **argv,pos target);//argc,argv为命令行参数，用于初始化节点；此接口接受目标点，并导航机器人
